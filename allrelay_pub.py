...
f = open(fname, 'r')
for line in f:
  pub = re.search(r'(Published bandwidth) (relay\d+[a-z]*)=(\d+)', line)
  if pub:
    e = pub.groups()[1]
...
for e in relays:
  ...
  del bands[1::2]
  max_iter = min(max_iter, len(bands))
  ans = [relay_band[e]] * len(bands)
  print e + ' ' + str(relay_band[e])
  print e + ' ' + str(bands)
  err_mag_percents[e] = [100.0 * abs(float(bands[i] - ans[i])) / ans[i] for i in range(max_iter)]

  plt.ylim(0,max(relay_band[e], max(bands[1:max_iter])) * 1.1)
  plt.plot(ans[1:max_iter],'r', label='True capacity')
  plt.plot(bands[1:max_iter],'b', label='Estimated capacity')
  plt.ylabel('weight')
  plt.xlabel('round')
